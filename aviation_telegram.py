    ...
    for f in RSS_FEEDS:
        try:
            ...
            for e in feed.entries[:3]:
                articles.append({"source": f["name"], "title": e.get("title",""), "summary": e.get("summary", e.get("description",""))[:400]})
...
def main():
    ...
    print("✍️ 해외 브리핑 작성 중...")
    m1 = client.messages.create(model="claude-sonnet-4-6", max_tokens=1200,
        messages=[{"role":"user","content":f"항공 인증 전문가. 오늘({today}) 해외 항공 뉴스 텔레그램 메시지 작성. 구체적 사실 기반.\n\n[RSS]\n{news_text}\n\n[웹검색]\n{global_search}\n\n형식:\n✈️ *AW항공브리핑 해외편 | {today}*\n\n📌 *핵심 3줄*\n내용\n\n🚁 *UAM eVTOL 글로벌 동향*\n2~3문장\n\n📋 *FAA EASA 인증 변화*\n2~3문장\n\n💼 *글로벌 비즈니스 투자*\n2문장\n\n_AW인증솔루션_"}])
    print("✍️ 국내 브리핑 작성 중...")
    m2 = client.messages.create(model="claude-sonnet-4-6", max_tokens=1200,
